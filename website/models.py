...
class Product(db.Model):
    ...
    likeness_points = db.Column(db.Integer, nullable=True)

    carts = db.relationship('Cart', backref=db.backref('Product', lazy=True), cascade="all, delete-orphan")
    orders = db.relationship('Order', backref=db.backref('Product', lazy=True), cascade="all, delete-orphan")

    def __str__(self):
        return 'Product %r' % Product.product_name
